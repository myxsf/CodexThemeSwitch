...
deploy_project() {
  ...
  if [ -f "$PROJECT_ROOT/../themes/catalog.json" ]; then
    /bin/mkdir -p "$temporary/themes"
    /bin/cp "$PROJECT_ROOT/../themes/catalog.json" "$temporary/themes/catalog.json"
  fi
...
seed_bundled_themes() {
  local source id staged destination previous
  [ -d "$PROJECT_ROOT/themes" ] || fail "Bundled themes directory is missing."
  for source in "$PROJECT_ROOT"/themes/*; do
    [ -d "$source" ] || continue
    id="$(/usr/bin/basename "$source")"
    staged="$THEMES_ROOT/.${id}.installing.$$"
    destination="$THEMES_ROOT/$id"
    previous="$THEMES_ROOT/.${id}.previous.$$"
    /bin/rm -rf "$staged" "$previous"
    /bin/mkdir -p "$staged"
    /usr/bin/rsync -a "$source/" "$staged/"
    "$NODE" "$INJECTOR" --check-payload --theme-dir "$staged" >/dev/null \
      || fail "Bundled theme validation failed: $id"
    [ -e "$destination" ] && /bin/mv "$destination" "$previous"
    if ! /bin/mv "$staged" "$destination"; then
      [ -e "$previous" ] && /bin/mv "$previous" "$destination"
      fail "Could not install bundled theme: $id"
    fi
    /bin/rm -rf "$previous"
  done
}
...
if [ ! -f "$THEME_DIR/theme.json" ]; then
  "$SCRIPT_DIR/switch-theme-macos.sh" --id skin-05 --no-apply
fi
...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  switch_script="$(shell_quote "$SCRIPT_DIR/switch-theme-macos.sh")"
  ...
  write_launcher "$HOME/Desktop/Codex Theme - Inspiration Prince.command" "exec $switch_script --id prince"
  write_launcher "$HOME/Desktop/Codex Theme - Retro QQ 2007.command" "exec $switch_script --id qq2007"
fi
