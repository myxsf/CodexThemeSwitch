...
if [ "$(active_theme_id 2>/dev/null || true)" = "original" ]; then
  if [ -f "$STATE_PATH" ]; then stop_recorded_injector || true; fi
  ...
  printf 'Codex opened with the original appearance.\n'
  exit 0
fi
...
if [ "$DEBUG_READY" = "false" ]; then
  ...
  # Activate the instance already launched with CDP. `-n` here would create a
  # second Codex process with a competing renderer/debug endpoint.
...
# A verified CDP endpoint can belong to a background-only Codex process. The
# wardrobe's “Open Codex” action must still activate a real visible window.
...
if "$NODE" "$INJECTOR" --verify --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 20000 \
  >"$LAST_VERIFY_JSON" 2>"$LAST_VERIFY_ERROR_LOG"; then
  verify_code=0
else
  verify_code=$?
fi
...
if [ "$verify_code" -ne 0 ]; then
  # Strict visual verification can lag behind a Codex DOM update. If the
  # trusted endpoint and watcher are healthy, preserve the running theme and
  # keep the diagnostic artifacts for the wardrobe/doctor.
  if /bin/kill -0 "$INJECTOR_PID" 2>/dev/null && verified_cdp_endpoint "$PORT"; then
    printf 'Codex Dream Skin Studio %s is active on port %s; strict visual verification is pending.\n' "$SKIN_VERSION" "$PORT"
    exit 0
  fi
  /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || /bin/kill -TERM "$INJECTOR_PID" 2>/dev/null || true
  /bin/rm -f "$STATE_PATH"
  fail "Injection could not be kept alive; see $LAST_VERIFY_ERROR_LOG and $INJECTOR_ERROR_LOG"
fi
